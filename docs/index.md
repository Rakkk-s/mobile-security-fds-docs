...
단순한 위협 나열이 아닌, 실제 발생한 피해 사례와 악성 앱 분석 결과를 바탕으로 공격 시나리오를 재구성하고
각 단계에서 관측 가능한 탐지 데이터를 룰로 환원하여, 실제 금융 서비스 환경에 적용 가능한 대응 설계를 제시합니다.
...
> 7주 과정으로 진행되며, 각 단계의 결과물이 순차적으로 본 문서에 반영됩니다.
...
-   **이호영** · PM
    ...
    프로젝트 총괄, 위협 유형 분석, 공격 시나리오 설계, 탐지 데이터 분석, FDS 룰 설계
